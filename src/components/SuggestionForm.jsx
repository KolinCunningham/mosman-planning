    ...
    <div className="max-w-3xl mx-auto space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm">Complete all sections — audit runs automatically</p>
        </div>
      ...
      <div className="flex gap-1">
        {sections.map((s, i) => (
          <button key={i} onClick={() => setSection(i)}
            className={`flex-1 h-1.5 rounded transition-colors ${i === section ? 'bg-mosman-pink' : i < section ? 'bg-mosman-teal' : 'bg-mosman-border'}`}
          />
        ))}
      </div>
        ...
        {sections.map((s, i) => (
          <button key={i} onClick={() => setSection(i)}
            className={`px-3 py-1 rounded text-sm transition-colors ${
              i === section ? 'bg-mosman-pink text-white' : 'bg-mosman-border text-slate-400 hover:text-white'
            }`}>
            {s.label}
          </button>
        ))}
      ...
      <div className="bg-mosman-card border border-mosman-border rounded-xl p-5 space-y-4">
...
const sel = "w-full bg-mosman-dark border border-mosman-border rounded-lg px-3 py-2 text-slate-200 text-sm focus:outline-none focus:border-mosman-teal"
  ...
  return <>
    ...
    <Field label="Map Change Required?">
      <label className="flex items-center gap-2 cursor-pointer">
        ...
        <span className="text-slate-300 text-sm">This suggestion requires a LEP map modification</span>
      </label>
    </Field>
  </>
}
...
function giveTake(form, set) {
  return <>
    <div className="p-3 bg-mosman-dark rounded-lg border border-mosman-border text-xs text-slate-400 mb-2">
...
function principles(form, setNested) {
  return <>
    <p className="text-slate-400 text-sm">Rate this suggestion against all 7 Masterplan Principles.</p>
    {MASTERPLAN_PRINCIPLES.map(p => (
      <div key={p} className="flex items-center justify-between">
        <span className="text-slate-300 text-sm flex-1">{p}</span>
...
function audit(form, setNested, score, total) {
  return <>
    <div className="flex items-center justify-between mb-2">
      <p className="text-slate-400 text-sm">Check each item when complete. All 15 must pass before logging.</p>
    ...
    <div className="space-y-2">
      {AUDIT_ITEMS.map(item => (
        <label key={item} className="flex items-start gap-3 cursor-pointer group">
          ...
          />
          <span className={`text-sm ${form.auditChecks[item] ? 'text-green-300 line-through opacity-60' : 'text-slate-300'} group-hover:text-white transition-colors`}>
            {item}
          </span>
        </label>
      ))}
